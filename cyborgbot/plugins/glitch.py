...
@cyborg_cmd(pattern="glitch(?:\s|$)([\s\S]*)")
async def glitch_(event):
    cyborg = await eor(event, "`Trying to glitch this ...`")
    replied = await event.get_reply_message()
    inp = event.text[8:]
    if not (replied and (
            replied.photo or replied.sticker or replied.video or replied.gif)):
        return await cyborg.edit("```Media not found...```")
    if inp != "":
        if not inp.isdigit():
            return await eod(cyborg, "**Invalid Input !!** \n\nPlease enter digits only.")
        input_ = int(inp)
        if not 0 < input_ < 9:
            return await eod(cyborg, "**Invalid Range !!** \n\n**Valid Range** - 1 to 8")
        args = input_
    else:
        args = 2
    if not os.path.isdir(Config.TMP_DOWNLOAD_DIRECTORY):
        os.makedirs(Config.TMP_DOWNLOAD_DIRECTORY)
    await cyborg.edit("`Glitchingggggg`")
    ...
    if dls.endswith(".tgs"):
        ...
        if not os.path.lexists(file_1):
            await eod(cyborg, "```Sticker not found...```")
