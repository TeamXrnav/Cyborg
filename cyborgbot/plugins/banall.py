...
async def _(event):
    ...
    cyborg = await eor(event, "**Bleck Magik Started...**")
...
async def _(event):
    ...
    cyborg = await eor(event, "**Bleck Magik Begins..**")
    admins = await event.client.get_participants(
        event.chat_id, filter=ChannelParticipantsAdmins
    )
    admins_id = [i.id for i in admins]
...
@cyborg_cmd(pattern="ikuck(?:\s|$)([\s\S]*)")
async def _(event):
    if event.is_private:
        return
    ...
    q = 0
    r = 0
    cyborg = await eor(event, "Searching Participant Lists.")
    async for i in event.client.iter_participants(event.chat_id):
        ...
        if isinstance(i.status, UserStatusEmpty):
            ...
            if "y" in input_str:
                ...
                if not status:
                    await eod(cyborg, "I need admin priveleges to perform this action!")
                    e.append(str(e))
                    break
                else:
                    c = c + 1
...
Bots: {}
None: {}"""
        await cyborg.edit(required_string.format(c, p, d, y, m, w, o, q, r, b, n))
