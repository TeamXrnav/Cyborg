...
async def _(event):
    ...
    cyborg_memtion = cid[2]
    if event.reply_to_msg_id and input_str == "":
        reply_message = await event.get_reply_message()
        cyborg = await eor(event, "`Analysing...`")
        ...
        try:
            ...
            await cyborg.edit("`Removing background of this media`")
            file_name = toimage(file_name)
            response = ReTrieveFile(file_name)
            os.remove(file_name)
    elif input_str:
        cyborg = await eor(event, "`Removing Background of this media`")
    ...
    remove_bg_image = "CyborgBot.png"
    if "image" in contentType:
        with open("CyborgBot.png", "wb") as removed_bg_file:
            removed_bg_file.write(response.content)
    else:
        await eod(cyborg, f"`{response.content.decode('UTF-8')}`")
        return
    if flag and flag == "-s":
        file = tosticker(remove_bg_image, filename="CyborgBot.webp")
    ...
    await event.client.send_file(
        event.chat_id,
        file,
        caption=f"**Background removed by** {cyborg_memtion}",
        force_document=True,
        )
    await cyborg.delete()
