...
from cyborgbot.sql.gvar_sql import addgvar, gvarstat, delgvar
from . import *
...
cyborgbot = gvarstat("STICKER_PACKNAME")


@cyborg_cmd(pattern="kang(?:\s|$)([\s\S]*)")
async def kang(event):
    user = await event.client.get_me()
    ForGo10God, CYBORG_USER, cyborg_mention = await client_id(event)
    ...
    if message and message.media:
        if isinstance(message.media, MessageMediaPhoto):
            cyborg = await eor(event, f"`{random.choice(KANGING_STR)}`")
            photo = io.BytesIO()
    ...
    if photo:
        ...
        packname = f"Cyborgbot_{un_}_{pack}"
        packnick = (
            f"{cyborgbot} Vol.{pack}"
            if cyborgbot
            else f"{un}'s Cyborg Vol.{pack}"
        )
        cmd = "/newpack"
        file = io.BytesIO()
        ...
        if (
            ...
            async with event.client.conversation("@Stickers") as conv:
                ...
                while "120" in x.text:
                    pack += 1
                    packname = f"CyborgBot_{un_}_pack_{pack}"
                    packnick = (
                        f"{cyborgbot} Vol.{pack}"
                        if cyborgbot
                        else f"{un}'s Cyborg Vol.{pack}"
            ...
            await cyborg.edit("`Preparing a new pack....`")
            async with event.client.conversation("Stickers") as conv:
                await conv.send_message(cmd)
                await conv.get_response()
                # Ensure user doesn't get spamming notifications
...
async def get_pack_info(event):
    ...
    try:
        ...
        cyborg = await eor(event, "`Fetching details of the sticker pack, please wait..`")
    except BaseException:
        await eod(event, "`This is not a sticker. Reply to a sticker.`")
        return

    if not isinstance(stickerset_attr, DocumentAttributeSticker):
        await cyborg.edit("`This is not a sticker. Reply to a sticker.`")
        return
    ...
    await cyborg.edit(OUTPUT)


@cyborg_cmd(pattern="delst(?:\s|$)([\s\S]*)")
async def _(event):
    ...
    cyborg = await eor(event, "🥴 `Deleting sticker...`")
    async with event.client.conversation(chat) as conv:
        try:
            ...
            await cyborg.edit("Please unblock @Stickers and try again")
            return
        await cyborg.edit(response.message)


@cyborg_cmd(pattern="editst(?:\s|$)([\s\S]*)")
async def _(event):
    if not event.reply_to_msg_id:
        await eod(event, "`Reply to any user's message.`")
        return
    ...
    chat = "@Stickers"
    cyborg = await eor(event, "📝 `Editing sticker emoji...`")
    if hel_ == "":
        await cyborg.edit("**🤧 Nashe me hai kya lawde**")
    else:
        async with event.client.conversation(chat) as conv:
            try:
                response = conv.wait_event(
                ...
                await cyborg.edit("Please unblock @Stickers and try again")
                return
            await cyborg.edit(f"{response.message}")


@cyborg_cmd(pattern="pkang(?:\s|$)([\s\S]*)")
async def _(event):
    ...
    if cyborg == "":
        pname = f"{un}'s Cyborg Pack"
    else:
        pname = cyborg
    if rply and rply.media and rply.media.document.mime_type == "image/webp":
        cyborg_id = rply.media.document.attributes[1].stickerset.id
        cyborg_hash = rply.media.document.attributes[1].stickerset.access_hash
        got_stcr = await event.client(
            functions.messages.GetStickerSetRequest(
                stickerset=types.InputStickerSetID(id=cyborg_id, access_hash=cyborg_hash)
            )
        )
        stcrs = []
        ...
        try:
            create_st = await tbot(
                functions.stickers.CreateStickerSetRequest(
                    user_id=ForGo10God,
                    title=pname,
                    short_name=f"cyborg_{un_}_V{pack}_by_{bot_un}",
                    stickers=stcrs,
                )
            )
...
async def sticklet(event):
    ...
    FONT_FILE = await get_font_file(event.client, "@CyborgFonts")
    ...
    image_stream.name = "Cyborgbot.webp"
...
@cyborg_cmd(pattern="waifu(?:\s|$)([\s\S]*)")
