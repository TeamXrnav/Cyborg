...
from cyborgbot.sql import pmpermit_sql as pm_sql
from . import *

WARN_PIC = Config.PMPERMIT_PIC or "https://telegra.ph/file/d09cdb53e72cd7215892e.jpg"
PM_WARNS = {}
PREV_REPLY_MESSAGE = {}
PM_ON_OFF = Config.PM_PERMIT
CSTM_PMP = Config.CUSTOM_PMPERMIT or "**You Have Trespassed To My Master's PM!\nThis Is Illegal And Regarded As Crime.**"
cyborg_ZERO = "Go get some sleep retard. \n\n**Blocked !!**"


@cyborg_cmd(pattern="unblock$")
async def unb(event):
    if event.is_private:
...
if PM_ON_OFF != "DISABLE":
    ...
    async def on_new_private_message(event):
        if not event.is_private:
            return
        cid = await client_id(event)
        ForGo10God, cyborg_mention = cid[0], cid[2]
        cyborg_FIRST = "**☠️ Cʏʙᴏʀɢʙᴏᴛ Pʀɪᴠᴀᴛᴇ Sᴇᴄᴜʀɪᴛʏ Pʀᴏᴛᴏᴄᴏʟ ☠️**\n\nThis is to inform you that {} is currently unavailable.\nThis is an automated message.\n\n{}\n\n**Please Choose Why You Are Here!!**".format(cyborg_mention, CSTM_PMP)
    ...
    async def do_pm_permit_action(chat_ids, event):
        ...
        if PM_WARNS[chat_ids] == Config.MAX_SPAM:
            r = await event.reply(cyborg_ZERO)
...
CmdHelp("pm_permit").add_command(
  ...
  "listapproved", None, "Sends the list of all users approved by Cyborg"
).add_info(
  "PM SECURITY"
).add_warning(
  "✅ Harmless Module."
