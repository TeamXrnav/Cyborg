...
@cyborg_cmd(pattern="kickme$")
async def leave(e):
        await e.edit("😪 **KThnxBye** See u all in cyborg!!")
        time.sleep(1)
        if "-" in str(e.chat_id):
...
@cyborg_cmd(pattern="dc$")
async def _(event):
    result = await event.client(functions.help.GetNearestDcRequest())
    await eor(event, result.stringify())


@cyborg_cmd(pattern="config$")
async def _(event):
...
@cyborg_cmd(pattern="vars$")
async def lst(event):
    cyborg = await eor(event, "Getting configs list...")
...
CmdHelp("bot").add_command(
    ...
    "schd", "<secs> - <message>", "Sends your message in given secs", "schd 10 - cyborgo"
).add_command(
