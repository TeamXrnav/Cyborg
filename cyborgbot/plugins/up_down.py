...
@cyborg_cmd(pattern="webup(?:\s|$)([\s\S]*)")
...
@cyborg_cmd(pattern="upld_dir(?:\s|$)([\s\S]*)")
async def uploadir(event):
    input_str = event.pattern_match.group(1)
    if os.path.exists(input_str):
        cyborg = await eor(event, "Downloading Using Userbot Server....")
        lst_of_files = []
        ...
        uploaded = 0
        await cyborg.edit("Found {} files. Uploading will start soon. Please wait!".format(len(lst_of_files)))
        for single_file in lst_of_files:
            if os.path.exists(single_file):
                ...
                uploaded = uploaded + 1
        await cyborg.edit("Uploaded {} files successfully !!".format(uploaded))
    else:
        await cyborg.edit("404: Directory Not Found")

@cyborg_cmd(pattern="upload(?:\s|$)([\s\S]*)")
async def upload(event):
    ...
    if os.path.exists(input_str):
        ...
        await cyborg.edit("Uploaded successfully !!")
    else:
        await cyborg.edit("404: File Not Found")
...
@cyborg_cmd(pattern="upld_as_(stm|vnr|all)(?:\s|$)([\s\S]*)")
    ...
    if os.path.exists(file_name):
        ...
        try:
            if supports_streaming:
                ...
                await cyborg.edit("TBD: Not (yet) Implemented")
                return
            os.remove(thumb)
            await cyborg.edit("Uploaded successfully !!")
        except FileNotFoundError as err:
            await cyborg.edit(str(err))
    else:
        await cyborg.edit("404: File Not Found")

@cyborg_cmd(pattern="download(?:\s|$)([\s\S]*)")
async def _(event):
    cyborg = await eor(event, "`Processing ...`")
    ...
    if event.reply_to_msg_id:
        ...
        try:
            ...
            downloaded_file_name = await event.client.download_media(
                ...
                progress_callback=lambda d, t: asyncio.get_event_loop().create_task(
                    progress(d, t, cyborg, c_time, "trying to download")
                ),
            )
        except Exception as e:  # pylint:disable=C0103,W0703
            await cyborg.edit(str(e))
        else:
            end = datetime.datetime.now()
            ms = (end - start).seconds
            await cyborg.edit(f"**•  Downloaded in {ms} seconds.**\n**•  Downloaded to :- ** `{downloaded_file_name}`\n**•  Downloaded by :-** {cyborg_mention}")
    elif input_str:
        start = datetime.datetime.now()
        url = input_str
        ...
        while not downloader.isFinished():
            ...
            try:
                ...
                if round(diff % 10.00) == 0 and current_message != display_message:
                    await cyborg.edit(current_message)
        ...
        if downloader.isSuccessful():
            ...
            await cyborg.edit("Incorrect URL\n {}".format(input_str))
    else:
        await cyborg.edit("Reply to a message to download to my local server.")
