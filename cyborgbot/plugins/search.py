...
@cyborg_cmd(pattern="ytube(?:\s|$)([\s\S]*)")
async def _(event):
...
@cyborg_cmd(pattern="lmlog(?:\s|$)([\s\S]*)")
async def _(event):
    input_str = event.pattern_match.group(1)
...
@cyborg_cmd(pattern="rchiv(?:\s|$)([\s\S]*)")
async def _(event):
    input_str = event.pattern_match.group(1)
...
CmdHelp("search").add_command(
  ...
  "ytube", "<query>", "Gives you the link of given query from youtube"
).add_command(
