...
async def _(event):
    ...
    if user.first_name.startswith(OFFLINE_TAG):
        ...
        return
    cyborg = await eor(event, "**Changing Profile to Offline...**")
    ...
    if photo:
        ...
        try:
            ...
            await eod(cyborg, str(e))
        else:
            await eod(cyborg, "**Changed profile to OffLine.**")
    try:
    ...
    try:
        ...
        await eod(cyborg, result)
    except Exception as e:
        await eod(cyborg, str(e))


@cyborg_cmd(pattern="online$")
async def _(event):
    user_it = "me"
    user = await event.client.get_entity(user_it)
    if user.first_name.startswith(OFFLINE_TAG):
    ...
    if photo:
        ...
        try:
            ...
            await eod(cyborg, "**Changed profile to Online.**")
    try:
        os.system("rm -fr donottouch.jpg")
    except Exception as e:
        LOGS.warn(str(e))
...
@cyborg_cmd(pattern="pname(?:\s|$)([\s\S]*)")
async def _(event):
    names = event.text[7:]
    first_name = names
    last_name = ""
...
@cyborg_cmd(pattern="ppic$")
async def _(event):
    reply_message = await event.get_reply_message()
    cyborg = await eor(event, "Downloading Profile Picture to my local ...")
    if not os.path.isdir(Config.TMP_DOWNLOAD_DIRECTORY):
        os.makedirs(Config.TMP_DOWNLOAD_DIRECTORY)
    photo = None
    try:
        ...
        if photo:
            await cyborg.edit("Uploading profile picture...")
            file = await event.client.upload_file(photo)
            try:
                await event.client(functions.photos.UploadProfilePhotoRequest(file))
            except Exception as e:
...
@cyborg_cmd(pattern="username(?:\s|$)([\s\S]*)")
...
async def count(event):
    ...
    await cyborg.edit(result, parse_mode="HTML")


@cyborg_cmd(pattern="delpfp$")
...
@cyborg_cmd(pattern="myusernames$")
async def _(event):
