...
@cyborg_cmd(pattern="thug$")
async def _(event):
    if not event.reply_to_msg_id:
        return await eod(event, "Reply to a image...")
    cyborg = await eor(event, "`Converting To thug Image..`")
    await event.get_reply_message()
    img = await convert_to_image(event, event.client)
    imagePath = img
    maskPath = "./cyborgbot/resources/pics/mask (1).png"
    cascPath = "./cyborgbot/resources/xmls/haarcascade_frontalface_default.xml"
...
@cyborg_cmd(pattern="trigger$")
async def _(event):
    cyborg = await eor(event, "`Trigggggggggerrr`")
...
@cyborg_cmd(pattern="geyy$")
async def _(event):
    cyborg = await eor(event, "`Geyyyy`")
    owo = await event.get_reply_message()
    img = await convert_to_image(event, event.client)
    url_s = upload_file(img)
    imglink = f"https://telegra.ph{url_s[0]}"
...
@cyborg_cmd(pattern="pix$")
async def _(event):
    cyborg = await eor(event, "`Pixing This Image.`")
    owo = await event.get_reply_message()
    img = await convert_to_image(event, event.client)
    url_s = upload_file(img)
    imglink = f"https://telegra.ph{url_s[0]}"
...
@cyborg_cmd(pattern="ytc(?:\s|$)([\s\S]*)")
async def _(event):
    cyborg = await eor(event, "`Making a youthuub comment...`")
    owo = await event.get_reply_message()
