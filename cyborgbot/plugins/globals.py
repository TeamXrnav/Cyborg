...
from cyborgbot.sql.gban_sql import is_gbanned, gbaner, ungbaner, all_gbanned
from cyborgbot.sql.gvar_sql import gvarstat
from cyborgbot.sql import gmute_sql as gsql
from . import *
...
@cyborg_cmd(pattern="gpro(?:\s|$)([\s\S]*)")
async def _(event):
    i = 0
    sender = await event.get_sender()
    me = await event.client.get_me()
    cyborg = await eor(event, "`Promoting globally...`")
    ...
    try:
        ...
        pass
    if me == user:
       k = await cyborg.edit("You can't promote yourself...")
       return
    try:
        if not rank:
            rank = "ㅤ"
    except:
        return await cyborg.edit("**ERROR !!**")
    if user:
        ...
        for x in telchanel:
          try:
             ...
             i += 1
             await cyborg.edit(f"**Promoting User in :**  `{i}` Chats...")
          except:
             pass
    else:
        await cyborg.edit(f"**Reply to a user !!**")
    await cyborg.edit(
...
@cyborg_cmd(pattern="gdem(?:\s|$)([\s\S]*)")
async def _(event):
    i = 0
    sender = await event.get_sender()
    me = await event.client.get_me()
    cyborg = await eor(event, "`Demoting Globally...`")
    my_mention = "[{}](tg://user?id={})".format(me.first_name, me.id)
    f"@{me.username}" if me.username else my_mention
    await event.get_chat()
    if event.is_private:
    ...
    try:
        ...
        pass
    if me == user:
       k = await cyborg.edit("You can't Demote yourself !!")
       return
    try:
        if not rank:
            rank = "ㅤ"
    ...
    if user:
        ...
        for x in telchanel:
          try:
             await event.client(EditAdminRequest(x, user, rgt, rank))
             i += 1
             await cyborg.edit(f"**Demoting Globally In Chats :** `{i}`")
          except:
             pass
    else:
        await cyborg.edit(f"**Reply to a user !!**")
...
@cyborg_cmd(pattern="gban(?:\s|$)([\s\S]*)")
async def _(event):
    cyborg = await eor(event, f"`Gbanning ...`")
    reason = ""
    ...
    if event.reply_to_msg_id:
        ...
        return await eod(cyborg, "**To gban a user i need a userid or reply to his/her message!!**")
    name = (await event.client.get_entity(userid)).first_name
    chats = 0
    if userid == ForGo10God:
        return await eod(cyborg, "🥴 **Nashe me hai kya lawde ‽**")
    if str(userid) in DEVLIST:
        return await eod(cyborg, "😑 **GBan my creator ?¿ Really‽**")
    ...
    if a is not None:
        ...
        gbpic = cjb
    gmsg = f"🥴 [{name}](tg://user?id={userid}) **beta majdur ko khodna 😪 aur** {cyborg_mention} **ko chodna... Kabhi sikhana nhi!! 😏**\n\n📍 Added to Gban Watch!!\n**🔰 Total Chats :**  `{chats}`"
    if reason != "":
        gmsg += f"\n**🔰 Reason :**  `{reason}`"
    ogmsg = f"[{name}](tg://user?id={userid}) **Is now GBanned by** {cyborg_mention} **in**  `{chats}`  **Chats!! 😏**\n\n**📍 Also Added to Gban Watch!!**"
    if reason != "":
        ogmsg += f"\n**🔰 Reason :**  `{reason}`"
    if Config.ABUSE == "ON":
        ...
        await cyborg.edit(ogmsg)


@cyborg_cmd(pattern="ungban(?:\s|$)([\s\S]*)")
async def _(event):
    cid = await client_id(event)
    ForGo10God, CYBORG_USER, cyborg_mention = cid[0], cid[1], cid[2]
    cyborg = await eor(event, "`Ungban in progress...`")
    if event.reply_to_msg_id:
        userid = (await event.get_reply_message()).sender_id
    elif event.pattern_match.group(1):
        ...
        return await eod(cyborg, "`Reply to a user or give their userid... `")
    name = (await event.client.get_entity(userid)).first_name
    chats = 0
    if not is_gbanned(userid):
        return await eod(cyborg, "`User is not gbanned.`")
    await cyborg.edit(f"Ungbaning in client 1... \n**Unbanned in :** `{chats}`")
    async for gfuck in event.client.iter_dialogs():
        if gfuck.is_group or gfuck.is_channel:
            try:
...
async def already(event):
    ...
    if len(gbanned_users) > 0:
        for user in gbanned_users:
            ...
            cyborg = int(hel)
            try:
                tity = await event.client.get_entity(cyborg)
                name = tity.first_name
            except ValueError:
                name = "User"
            GBANNED_LIST += f"📍 [{name}](tg://user?id={cyborg}) (`{cyborg}`)\n"
    else:
...
@cyborg_cmd(pattern="gkick(?:\s|$)([\s\S]*)")
async def gkick(event):
    cid = await client_id(event)
    ForGo10God, CYBORG_USER, cyborg_mention = cid[0], cid[1], cid[2]
    cyborg = await eor(event, f"`Kicking globally ...`")
    reply = await event.get_reply_message()
    if event.reply_to_msg_id:
        userid = (await event.get_reply_message()).sender_id
    elif event.pattern_match.group(1):
        ...
        return await eod(cyborg, "`Reply to some msg or add their id.`")
    name = (await event.client.get_entity(userid)).first_name
    chats = 0
    if userid == ForGo10God:
        return await eod(cyborg, "**🥴 Nashe me hai kya lawde!!**")
    if str(userid) in DEVLIST:
        return await eod(cyborg, "**😪 I'm not going to gkick my developer!!**")
    ...
    if Config.ABUSE == "ON":
        ...
        await cyborg.edit(gkmsg)


@cyborg_cmd(pattern="gmute(?:\s|$)([\s\S]*)")
async def gm(event):
    private = False
    cid = await client_id(event)
    ForGo10God, CYBORG_USER, cyborg_mention = cid[0], cid[1], cid[2]
...
@cyborg_cmd(pattern="ungmute(?:\s|$)([\s\S]*)")
