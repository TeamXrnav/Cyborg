...
async def _(event):
    ...
    if not reply_message.media:
        ...
        return
    chat = "@DrWebBot"
    cyborgevent = await eor(event, " `Scanning This media..... wait👀`")
    async with event.client.conversation(chat) as conv:
        try:
            ...
            await eod(cyborgevent, "`Please unblock `@DrWebBot `and try again`")
            return
        if response.text.startswith("Forward"):
            await eod(cyborgevent,
...
@cyborg_cmd(pattern="barcode(?:\s|$)([\s\S]*)")
async def _(event):
    ...
    try:
        ...
        await cyborgevent.edit(str(e))
        return
    end = datetime.datetime.now()
    ms = (end - start).seconds
    await cyborgevent.edit("Created BarCode in {} seconds🤓".format(ms))
    await asyncio.sleep(5)
    await cyborgevent.delete()


@cyborg_cmd(pattern="makeqr(?: |$)([\s\S]*)")
...
async def _(event):
    ...
    if len(input_sgra) == 3:
        ...
        await cyborg.edit(str(a))
    else:
        await eod(cyborg, f"SYNTAX: {hl}calendar YYYY.MM.DD")


@cyborg_cmd(pattern="currency(?:\s|$)([\s\S]*)")
async def _(event):
    input_str = event.pattern_match.group(1)
    input_sgra = input_str.split(" ")
    if len(input_sgra) == 3:
...
@cyborg_cmd(pattern="color(?:\s|$)([\s\S]*)")
async def _(event):
    input_str = event.pattern_match.group(1)
    message_id = None
...
@cyborg_cmd(pattern="xkcd(?:\s|$)([\s\S]*)")
async def _(event):
    cyborgevent = await eor(event, "`processiong...`")
    input_str = event.pattern_match.group(1)
    xkcd_id = None
    if input_str:
...
Year: {}""".format(
        ...
        await cyborgevent.edit(output_str, link_preview=True)
    else:
        await eod(cyborgevent, "xkcd n.{} not found!".format(xkcd_id))

@cyborg_cmd(pattern="dns(?:\s|$)([\s\S]*)")
async def _(event):
    input_str = event.pattern_match.group(1)
...
@cyborg_cmd(pattern="unshort(?:\s|$)([\s\S]*)")
async def _(event):
    input_str = event.pattern_match.group(1)
...
CmdHelp("tools").add_command(
  ...
  "currency", "<amount> <from> <to>", "Currency converter for CyborgBot", ".currency 10 usd inr"
).add_command(
