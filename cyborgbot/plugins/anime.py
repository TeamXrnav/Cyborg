...
async def _(event):
    ...
    try:
        ...
        await cyborg.edit(finals_)
    if os.path.exists(pic):
        os.remove(pic)


@cyborg_cmd(pattern="character(?:\s|$)([\s\S]*)")
async def _(event):
    query = event.text[11:]
    if query == "":
    ...
    try:
        ...
        await cyborg.delete(cap_text)
    if os.path.exists(img):
        os.remove(img)


@cyborg_cmd(pattern="fillers(?:\s|$)([\s\S]*)")
async def canon(event):
    cyborg = event.text[9:]
    if cyborg == "":
        return await eor(event, "`Give anime name to search filler episodes.`")
    nub = await eor(event, f"Searching Filler Episodes For `{cyborg}`")
    hel_ = search_filler(cyborg)
    if hel_ == {}:
        return await nub.edit(f"No filler found for `{cyborg}`")
    ...
    if len(list_) == 1:
        ...
        return
    cyborgbot = f"**📃 Filler Episode Lists :** \n\n"
    for i in list_:
        ...
        cyborgbot += f"• [{i}]({paste})\n"
    await nub.edit(cyborgbot)


@cyborg_cmd(pattern="airing(?:\s|$)([\s\S]*)")
async def _(event):
    query = event.text[8:]
    cyborg = await eor(event, f"__Searching airing details for__ `{query}`")
    if query == "":
        return await eod(cyborg, "Give anime name to seaech airing information.")
    vars_ = {"search": query}
    if query.isdigit():
    ...
    try:
        ...
        await cyborg.edit(out)
    if os.path.exists(coverImg):
        os.remove(coverImg)


@cyborg_cmd(pattern="aniuser(?:\s|$)([\s\S]*)")
async def _(event):
    query = event.text[9:]
    cyborg = await eor(event, "Searching user's Anilist Stats...")
    if query == "":
        return await cyborg.edit("No user found. Give anilist username.")
    qry = {"search": query}
    result = await get_user(qry)
    if len(result) == 1:
        return await eod(cyborg, result[0])
    pic, msg = result
    try:
...
CmdHelp("anime").add_command(
  ...
  "aniuser", "<anilist username>", "Searches for the Anilist Stats of the given user.", "aniuser meizcyborgboy"
).add_command(
