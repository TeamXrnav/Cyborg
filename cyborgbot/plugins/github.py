...
@cyborg_cmd(pattern="commit(?:\s|$)([\s\S]*)")
async def download(event):
    ...
    cyborgbot = await eor(event, "Processing ...")
    ...
    try:
        ...
        await eod(cyborgbot, str(e))
    else:
        end = datetime.datetime.now()
        ms = (end - start).seconds
        await event.delete()
        await cyborgbot.edit(
            "Downloaded to `{}` in {} seconds.".format(downloaded_file_name, ms)
        )
        await cyborgbot.edit("Committing to Github....")
        await git_commit(downloaded_file_name, path, branch, cyborgbot)


async def git_commit(file_name, path, branch, cyborgbot):
    content_list = []
    ...
    for i in content_list:
        ...
        if i == 'ContentFile(path="' + file_name + '")':
            return await cyborgbot.edit("`File Already Exists`")
            create_file = False
    path = path
    file_name = file_name
    if create_file == True:
        ...
        try:
            ...
            await eod(cyborgbot, "Cannot Upload File")
    else:
        return await eod(cyborgbot, "`Committed Suicide`")


@cyborg_cmd(pattern="github(?:\s|$)([\s\S]*)")
async def _(event):
...
CmdHelp("github").add_command(
  "commit", "<reply to a file> <path>|<branch>", "Uploads the file on github repo as provided in Heroku Config GIT_REPO_NAME. In short makes a commit to git repo from Userbot", "commit ./cyborgbot/plugins/example.py|master"
).add_command(
