...
async def parse_ocr_space_api(event):
    ...
    try:
        ...
        await eod(cyborg, "**Errors !!** \n`{}`\n**Report This to** {}\n\n`{}`".format(
                str(e), cyborg_grp, json.dumps(test_file, sort_keys=True, indent=4)
            )
        )
    else:
        await cyborg.edit("Read Document in {} seconds. \n{}".format(
                ProcessingTimeInMilliseconds, ParsedText
            )
        )
    os.remove(downloaded_file_name)
    await cyborg.edit(ParsedText)
