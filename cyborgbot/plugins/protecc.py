...
from cyborgbot.sql.waifu_sql import is_harem, add_grp, rm_grp, get_all_grp
from . import *
...
@cyborg_cmd(pattern="pt(?:\s|$)([\s\S]*)")
async def _(event):
    cyborg = await eor(event, "Hmm..")
    ...
    if event.reply_to_msg_id:
        ...
        OUTPUT_STR = """/protecc {prs_text}""".format(
            **locals())
    await cyborg.edit(OUTPUT_STR, parse_mode="HTML", link_preview=False)
        ...
        if int(grps.chat_id) == event.chat_id:
            try:
                ...
                try:
                    ...
                    pass
                cyborg = await event.client.send_message(event.chat_id, f"/protecc {text}")
                await sleep(2)
                await cyborg.delete()
                os.remove(dl)
            except:
...
@cyborg_cmd(pattern="adwaifu(?:\s|$)([\s\S]*)")
async def _(event):
...
@cyborg_cmd(pattern="rmwaifu(?:\s|$)([\s\S]*)")
async def _(event):
    if not event.is_group:
        await eod(event, "Autowaifu works in groups only !!")
        return
...
@cyborg_cmd(pattern="aw$")
async def _(event):
    cyborg = await eor(event, "Fetching Autowaifu chats...")
    all_grp = get_all_grp()
