...
@cyborg_cmd(pattern="love$")
async def love(e):
    txt = choice(LOVESTR)
    await eor(e, txt)

@cyborg_cmd(pattern="dhoka$")
async def katgya(e):
    txt = choice(DHOKA)
    await eor(e, txt)

@cyborg_cmd(pattern="metoo$")
async def metoo(e):
    txt = choice(METOOSTR)
    await eor(e, txt)

@cyborg_cmd(pattern="gdnoon$")
async def noon(e):
    txt = choice(GDNOON)
    await eor(e, txt)

@cyborg_cmd(pattern="chase$")
async def police(e):
    txt = choice(CHASE_STR)
    await eor(e, txt)

@cyborg_cmd(pattern="congo$")
async def Sahih(e):
    txt = choice(CONGRATULATION)
    await eor(e, txt)

@cyborg_cmd(pattern="qhi$")
async def hoi(e):
    txt = choice(HELLOSTR)
    await eor(e, txt)

@cyborg_cmd(pattern="gdbye$")
async def bhago(e):
    txt = choice(BYESTR)
    await eor(e, txt)

@cyborg_cmd(pattern="gdnyt$")
async def night(e):
    txt = choice(GDNIGHT)
    await eor(e, txt)

@cyborg_cmd(pattern="gdmng$")
async def morning(e):
    txt = choice(GDMORNING)
    await eor(e, txt)

@cyborg_cmd(pattern="quote(?:\s|$)([\s\S]*)")
async def quote_search(event):
    ...
    if response is not None:
        await cyborg.edit(f"`{response['text']}`")
    else:
        await eod(cyborg, "`Sorry Zero results found`")
