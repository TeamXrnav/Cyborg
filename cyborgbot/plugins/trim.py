...
FF_MPEG_DOWN_LOAD_MEDIA_PATH = "./trim/cyborgbot.media.ffmpeg"


@cyborg_cmd(pattern="tsave$")
async def ff_mpeg_trim_cmd(event):
    if not os.path.exists(FF_MPEG_DOWN_LOAD_MEDIA_PATH):
        ...
        if reply_message:
            ...
            cyborgevent = await eor(event, "`Saving the file...`")
            try:
                c_time = time.time()
                downloaded_file_name = await event.client.download_media(
                    ...
                    progress_callback=lambda d, t: asyncio.get_event_loop().create_task(
                        progress(d, t, cyborgevent, c_time, "trying to download")
                    ),
                )
...
async def ff_mpeg_trim_cmd(event):
    ...
    if len(cmt) == 3:
        ...
        if o is None:
            return await eod(cyborgevent, f"**Error : **`Can't complete the process`")
        try:
            c_time = time.time()
            await event.client.send_file(
                ...
                allow_cache=False,
                progress_callback=lambda d, t: asyncio.get_event_loop().create_task(
                    progress(d, t, cyborgevent, c_time, "trying to upload")
                ),
            )
            os.remove(o)
        except Exception as e:
            return await eod(cyborgevent, f"**Error : **`{e}`")
    elif len(cmt) == 2:
        ...
        await eod(cyborgevent, "RTFM")
        return
    end = datetime.datetime.now()
    ms = (end - start).seconds
    await eod(cyborgevent, f"`Completed Process in {ms} seconds`")


@cyborg_cmd(pattern="atrim(?:\s|$)([\s\S]*)")
async def ff_mpeg_trim_cmd(event):
    if not os.path.exists(FF_MPEG_DOWN_LOAD_MEDIA_PATH):
        await eod(event, f"A media file needs to be download, and save to the following path:  `{FF_MPEG_DOWN_LOAD_MEDIA_PATH}`")
        return
    ...
    ms = (end - start).seconds
    await eod(cyborgevent, f"`Completed Process in {ms} seconds`", 3)


@cyborg_cmd(pattern="tclear$")
async def ff_mpeg_trim_cmd(event):
    if not os.path.exists(FF_MPEG_DOWN_LOAD_MEDIA_PATH):
