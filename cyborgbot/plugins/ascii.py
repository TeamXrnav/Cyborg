...
@cyborg_cmd(pattern="ascii(?:\s|$)([\s\S]*)")
async def _(event):
    ...
    async with event.client.conversation(bot) as conv:
        try:
            ...
            return
    await event.client.send_file(event.chat_id, file=output_op, caption=f"ASCII art By :- {cyborg_mention}", force_document=False)
    await kraken.delete()  
    await event.client.delete_messages(
        conv.chat_id, [first.id, response.id, second.id, output_op.id]
    )

@cyborg_cmd(pattern="line(?:\s|$)([\s\S]*)")
async def _(event):
    if not event.reply_to_msg_id:
    ...
    async with event.client.conversation(bot) as conv:
        try:
            ...
            return
    await event.client.send_file(event.chat_id, file=output_op, caption=f"Lines By :- {cyborg_mention}", force_document=False)
    await kraken.delete()
    await event.client.delete_messages(
        conv.chat_id, [first.id, response.id, second.id, output_op.id]
    )
