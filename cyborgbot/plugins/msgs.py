...
async def all_groups_id(cyborg):
    cyborggroups = []
    async for dialog in cyborg.client.iter_dialogs():
        entity = dialog.entity
        if isinstance(entity, Channel) and entity.megagroup:
            cyborggroups.append(entity.id)
    return cyborggroups


@cyborg_cmd(pattern="frwd$")
async def _(event):
...
@cyborg_cmd(pattern="fpost(?:\s|$)([\s\S]*)")
async def _(event):
    global groupsid
    global msg_cache
    await event.delete()
