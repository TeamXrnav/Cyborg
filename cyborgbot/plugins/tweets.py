...
@cyborg_cmd(pattern="tweet(?:\s|$)([\s\S]*)")
async def nope(kraken):
    cyborg = kraken.text[7:]
    ...
    if not cyborg:
        if kraken.is_reply:
            ...
            return
    tweeter = await event.client.inline_query("TwitterStatusBot", f"{(deEmojify(cyborg))}")
...
@cyborg_cmd(pattern="trump(?:\s|$)([\s\S]*)")
...
@cyborg_cmd(pattern="pappu(?:\s|$)([\s\S]*)")
async def nekobot(event):
    text = event.text[7:]
    reply_to_id = event.message
    if event.reply_to_msg_id:
...
@cyborg_cmd(pattern="sunny(?:\s|$)([\s\S]*)")
async def nekobot(event):
    text = event.text[7:]
    reply_to_id = event.message
    if event.reply_to_msg_id:
...
@cyborg_cmd(pattern="johhny(?:\s|$)([\s\S]*)")
async def nekobot(event):
    text = event.text[8:]
    reply_to_id = event.message
    if event.reply_to_msg_id:
...
@cyborg_cmd(pattern="gandhi(?:\s|$)([\s\S]*)")
async def nekobot(event):
    text = event.text[8:]
    reply_to_id = event.message
    if event.reply_to_msg_id:
...
@cyborg_cmd(pattern="kanna(?:\s|$)([\s\S]*)")
async def nekobot(event):
    text = event.text[7:]
    reply_to_id = event.message
    if event.reply_to_msg_id:
