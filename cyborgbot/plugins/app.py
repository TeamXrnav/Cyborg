...
async def mod(event):
    ...
    if not cyborg:
        if event.is_reply:
            ...
            return
    uwu = await eor(event, f"Searching Mod for  `{cyborg}` ...")
    troll = await event.client.inline_query("PremiumAppBot", f"{(deEmojify(cyborg))}")
