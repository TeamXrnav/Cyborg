...
@cyborg_cmd(pattern="imdb(?:\s|$)([\s\S]*)")
async def _(event):
    reply_to = await reply_id(event)
    hel_ = await eor(event, "`Processing ...`")
    try:
        cyborg = event.pattern_match.group(1)
        await hel_.edit("__Searching for__ `{}`".format(cyborg))
        # Credits to catuserbot.
        # Ported to cyborgbot and beautifications 
        #Madhar Chode Banda Hai 😎 Jo Kang ker ne aya hai .
        movies = imdb.search_movie(cyborg)
...
<b>Director :</b> <code>{mov_director}</code>"""
        ...
        paste = await telegraph_paste(f"IMDb Movie Result for “ {cyborg} ”", telegraph_)
        ...
        await hel_.edit(f"__No movie found with name {cyborg}.__")
    except Exception as e:
        await hel_.edit(f"**Error:**\n__{e}__")


@cyborg_cmd(pattern="watch(?:\s|$)([\s\S]*)")
async def _(event):
    query = event.pattern_match.group(1)
    cyborg = await eor(event, "Finding Sites...")
