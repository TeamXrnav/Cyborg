...
cyborgbot_info = "https://raw.githubusercontent.com/TeamXrnav/Cyborg/master/cyborgbot-info.json"
urllib3.disable_warnings(urllib3.exceptions.InsecureRequestWarning)
requirements_path = os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(__file__))), "requirements.txt")


async def cyborg_info(cyborgbot_info):
    infos = requests.get(cyborgbot_info).json()
    _version = infos['CYBORGBOT-INFO']['version']
    _release = infos['CYBORGBOT-INFO']['release-date']
    _branch = infos['CYBORGBOT-INFO']['branch']
    _author = infos['CYBORGBOT-INFO']['author']
    _auturl = infos['CYBORGBOT-INFO']['author-url']
...
async def update(event, repo, ups_rem, ac_br):
    ...
    await eor(event, "✅ Successfully updated Cyborg!\n\nBot is restarting please wait for a minute.")
    args = [sys.executable, "-m", "cyborgbot"]
    os.execle(sys.executable, *args, os.environ)
    return


@cyborg_cmd(pattern="update(| now)$")
async def upstream(event):
    ...
    if changelog == "" and not force_update:
        _version, _release, _branch, _author, _auturl = await cyborg_info(cyborgbot_info)
        output_ = f"**Your Bot Version :** `{cyborg_ver}` \n**Owner :** {cyborg_mention} \n\n**Official CyborgBot Version :** `{_version}` \n**Release Date :** `{_release}` \n**Official Repo Branch :** `{_branch}` \n**Update By :** [{_author}]({_auturl})"
        if str(_version) not in str(cyborg_ver):
            output_ += f"\n\n**Do** `{hl}update build` **to update your CyborgBot to latest version.**"
...
async def deploy(event, repo, ups_rem, ac_br, txt):
    if HEROKU_API_KEY is not None:
        ...
        await event.edit(f"**Your Cyborg UserBot Is UpToDate**\n\n**Version :**  __{cyborg_ver}__\n**Oɯɳҽɾ :**  {cyborg_mention}")
    else:
