...
@cyborg_cmd(pattern="inviteall(?:\s|$)([\s\S]*)")
async def get_users(event):
    hel_ = event.text[11:]
    cyborg_chat = hel_.lower()
    restricted = ["@cyborgSupportchat", "@CyborgUpdates"]
    cyborg = await eor(event, f"__Inviting members from__ {hel_}")
    if cyborg_chat in restricted:
        await cyborg.edit("You can't Invite Members from there.")
        await event.client.send_message(-1001496036895, "Sorry for inviting members from here.")
        return
    kraken = await get_chatinfo(event)
    chat = await event.get_chat()
    if event.is_private:
        return await cyborg.edit("`Sorry, Cant add users here`")
    s = 0
    f = 0
    error = "None"
    await cyborg.edit("**INVITING USERS !!**")
    async for user in event.client.iter_participants(kraken.full_chat.id):
        try:
            ...
            error = str(e)
            f += 1
    return await cyborg.edit(
        f"**INVITING FINISHED** \n\n**Invited :**  `{s}` users \n**Failed :**  `{f}` users."
    )


@cyborg_cmd(pattern="add(?:\s|$)([\s\S]*)")
async def _(event):
    if (
