...
@cyborg_cmd(pattern="google(?:\s|$)([\s\S]*)")
async def google(event):
    input_str = event.pattern_match.group(1)
    if not input_str:
        return await eod(event, "`Give something to search..`")
    cyborg = await eor(event, "Searching...")
    gos = GoogleSearch()
    try:
...
{output}"""
    ...
    await cyborg.edit(f"**Google Search For** `{input_str}` \n[📌 See Results Here]({paste})", link_preview=False)


@cyborg_cmd(pattern="img(?:\s|$)([\s\S]*)")
async def img(event):
    sim = event.pattern_match.group(1)
    if not sim:
        return await eod(event, "`Give something to search...`")
    cyborg = await eor(event, f"Searching for `{sim}`...")
    if ";" in sim:
        try:
            lim = int(sim.split(";")[1])
            sim = sim.split(";")[0]
...
@cyborg_cmd(pattern="reverse(?:\s|$)([\s\S]*)")
async def _(event):
    ...
    link = alls["href"]
    text = alls.text
    await cyborg.edit(f"**Possible Results :** [{text}](google.com{link})")
...
async def gps(event):
    ...
    cyborg = await eor(event, "Finding😁")
    geolocator = Nominatim(user_agent="cyborgbot")
    ...
    if geoloc:
        ...
        await eod(cyborg, "I coudn't find it😫")
