...
@cyborg_cmd(pattern="rename(?:\s|$)([\s\S]*)")
async def _(event):
    input_str = event.text[8:]
    if input_str == "":
        return await eod(event, "Give a new file name..")
    cyborg = await eor(event, f"Renaming it to `{input_str}`")
    ...
    if event.reply_to_msg_id:
        ...
        if os.path.exists(downloaded_file_name):
            await cyborg.edit("Downloaded to `{}` in {} seconds.".format(downloaded_file_name, ms)
            )
        else:
            await eod(cyborg, "Error Occurred\n {}".format(input_str))
    else:
        await eod(cyborg, f"**Syntax Wrong !!** \n\n• `{hl}rename new file name` as reply to a Telegram file")

@cyborg_cmd(pattern="rnupload(?:\s|$)([\s\S]*)")
async def _(event):
    input_str = event.text[10:]
    cyborg = await eor(event, f"Renaming to `{input_str}`")
    if not os.path.isdir(Config.TMP_DOWNLOAD_DIRECTORY):
        os.makedirs(Config.TMP_DOWNLOAD_DIRECTORY)
    if event.reply_to_msg_id:
        start = datetime.datetime.now()
        ...
        if os.path.exists(downloaded_file_name):
            ...
            await cyborg.edit("Downloaded in {} seconds. Uploaded in {} seconds.".format(
                    ms_one, ms_two
                )
            )
        else:
        ...
        await cyborg.edit(f"**Syntax Wrong !!** \n\n• `{hl}rnupload new file name`")

@cyborg_cmd(pattern="rnsupload(?:\s|$)([\s\S]*)")
async def _(event):
    cyborg = await eor(event, "Rename & Upload as streamable format is in progress...")
        ...
        if os.path.exists(downloaded_file_name):
            ...
            try:
                await event.client.send_file(
                    ...
                    downloaded_file_name,
                    thumb=thumb,
                    caption="reuploaded by CyborgBot",
            ...
            await eod(cyborg, "File Not Found {}".format(input_str))
    else:
