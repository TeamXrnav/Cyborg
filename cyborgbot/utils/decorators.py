...
from cyborgbot.clients import H2, H3, H4, H5
...
def admin_cmd(pattern=None, command=None, **args):
    ...
    if pattern is not None:
        if pattern.startswith(r"\#"):
            ...
            if len(Config.HANDLER) == 2:
                cyborgreg = "^" + Config.HANDLER
                reg = Config.HANDLER[1]
            elif len(Config.HANDLER) == 1:
                cyborgreg = "^\\" + Config.HANDLER
                reg = Config.HANDLER
            args["pattern"] = re.compile(cyborgreg + pattern)
    ...
    # blacklisted chats. 
    # cyborgbot will not respond in these chats.
