...
from cyborgbot import LOGS, bot
...
async def make_gif(event, file):
    ...
    async with event.client.conversation(chat) as conv:
        try:
            ...
            cyborgresponse = response if response.media else await conv.get_response()
            await event.client.send_read_acknowledge(conv.chat_id)
            cyborgfile = await event.client.download_media(cyborgresponse, "./temp")
            return await unzip(cyborgfile)
