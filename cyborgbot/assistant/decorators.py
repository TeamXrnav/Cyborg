...
def cyborgbot_cmd(add_cmd, is_args=False):
    def cmd(func):
        cyborgbot = bot.tgbot
        if is_args:
...
def is_admin():
    def decorator(func):
        ...
        async def wrapper(event):
            cyborgbot = bot.tgbot
            perms = await cyborgbot.get_permissions(event.chat_id, event.sender_id)
...
def is_bot_admin():
    def decorator(func):
        ...
        async def wrapper(event):
            cyborgbot = bot.tgbot
            boat = await cyborgbot.get_me()
            perms = await cyborgbot.get_permissions(event.chat_id, boat)
            if perms.is_admin:
                await func(event)
            else:
